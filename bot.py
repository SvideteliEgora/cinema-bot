...
from afisha_parser import update_data
...
sht = gs.open_by_url(
    "https://docs.google.com/spreadsheets/d/12ccv7iv0kmbs8i2sbFy7FX6gETAc5_r2kAx1AoAVvu8/edit#gid=245321756"
)
...
for item in dt_list1:
    ...
    for dt in item[2:]:
        if dt != "":
            schedule_data_from_list1[cinema][movie].append(
                datetime.fromtimestamp(float(dt))
            )
...
# murkups
kb = [[KeyboardButton(text="Дата✅"), KeyboardButton(text="Кинотеатр✅")]]
start_kb = ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True)


async def ikb_cinemas(
    cinemas: list, all_cinemas_titles: list | tuple, current_page=1
) -> InlineKeyboardMarkup:
    ...
    for cin in cinemas[:buttons_on_page]:
        builder.button(
            text=cin, callback_data=f"cinema_{all_cinemas_titles.index(cin)}"
        )
    builder.adjust(1)

    if len(cinemas) > buttons_on_page:
        builder.button(
            text=f"{current_page} / {pages_count}", callback_data="pages_count"
        )
        builder.adjust(1)
    ...
    if current_page > 1:
        builder = InlineKeyboardBuilder()
        for cin in cinemas[
            (current_page - 1) * buttons_on_page : current_page * buttons_on_page
        ]:
            builder.button(
                text=cin, callback_data=f"cinema_{all_cinemas_titles.index(cin)}"
            )
...
async def ikb_movies(
    movies_data: list | tuple, all_movies_titles: list | tuple
) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()

    for movie in movies_data:
        builder.button(
            text=f"{movie}", callback_data=f"movie_{all_movies_titles.index(movie)}"
        )
    builder.adjust(1)
...
async def get_movies_and_showtime(
    schedule_data: dict[dict[str]], cinema: str, user_date: datetime
) -> tuple:
...
async def send_movie_schedule(
    callback_query, movie_and_showtime, user_date, user_cinema, all_movies_titles
) -> None:
            ...
            ikb = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text=movie,
                            callback_data=f"movie_{all_movies_titles.index(movie)}",
                        )
                    ]
                ]
            )
            await callback_query.message.answer(
                text=f'Дата: {user_date.strftime("%d.%m.%Y")}\n'
...
async def command_start_handler(message: Message) -> None:
    ...
    await message.reply(
        text=f"Приветствую, {hbold(message.from_user.full_name)}!\n"
        f"Я помогу узнать что сейчас показывают в Минских кинотеатрах.\n\n"
        f"Выбери начальный критерий для поиска:\n"
        f"- Дата✅\n"
        f"- Кинотеатр✅\n"
        f"- Или же напиши мне название фильма.\n\n"
        f"Следуй подсказкам и приятного просмотра!",
...
async def date_handler(message: Message) -> None:
    if not selected_user_movie.get(message.from_user.id):
        await clear_user_selections(message.from_user.id)

    await message.answer(
        text="Выберите дату:", reply_markup=await SimpleCalendar().start_calendar()
    )
...
async def cinema_handler(message: Message) -> None:
    ...
    await message.answer(
        ...
        reply_markup=await ikb_cinemas(
            cinemas=all_cinemas_titles, all_cinemas_titles=all_cinemas_titles
        ),
    )
...
async def handler_text_messages(message: Message):
    ...
    for movie in all_movies_titles:
        if fuzz.WRatio(movie, message.text) == 100:
            ...
            await message.answer(text="Фильм по вашему запросу:", reply_markup=ikb)
        elif fuzz.WRatio(movie, message.text) >= 85:
            movies.append(movie)

    if movies:
        await message.answer(
            text="Фильмы по вашему запросу:",
            reply_markup=await ikb_movies(
                movies_data=movies, all_movies_titles=all_movies_titles
            ),
        )
    else:
    ...
    if selected:
        ...
        if user_date < current_date:
            await callback_query.message.edit_text(
                text="Выбранная вами дата уже прошла, пожалуйста, выберите другую дату:",
        ...
        if user_cinema:
            movie_and_showtime = await get_movies_and_showtime(
                schedule_data_from_list1, user_cinema, user_date
            )
            await send_movie_schedule(
                callback_query,
                movie_and_showtime,
                user_date,
                user_cinema,
            ...
            await callback_query.message.edit_text(
                text=f"Теперь выберите кинотеатр:",
                reply_markup=await ikb_cinemas(
                    cinemas=all_cinemas_titles, all_cinemas_titles=all_cinemas_titles
                ),
            )
...
async def cinema_cb(callback_query: CallbackQuery):
    ...
    if user_date:
        ...
        if movie_and_showtime:
            ...
            await callback_query.message.answer(
                text=f'К сожалению, в этот день в кинотеатре: "{user_cinema}" нет показов.'
            )
        ...
        if date_and_time:
            ...
            return
        await callback_query.message.answer(
            text=f'К сожалению, фильм "{user_movie}" в настоящее время не представлен в кинотеатре "{user_cinema}".'
        )
    else:
        await callback_query.message.edit_text(
            text="Теперь выберите дату:",
            reply_markup=await SimpleCalendar().start_calendar(),
        )
...
async def movie_cb(callback_query: CallbackQuery):
    ...
    if not selected_user_cinema.get(
        callback_query.from_user.id
    ) and not selected_user_date.get(callback_query.from_user.id):
        await callback_query.message.answer(
            text="Дата✅ - для выбора даты сеанса.\n"
